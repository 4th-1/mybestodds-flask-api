    ...
    try:
        ...
        if initials and mmfsn_data:
            try:
                ...
                profile_path = os.path.join(mmfsn_dir, f"{initials}_mmfsn.json")
                ...
                logger.info(f"MMFSN profile written for {initials} ({len(mmfsn_clean['Cash3'])} Cash3, {len(mmfsn_clean['Cash4'])} Cash4)")
            except Exception as _e:
                logger.warning(f"MMFSN profile generation failed for {initials}: {_e}")
