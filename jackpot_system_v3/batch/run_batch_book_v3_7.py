"""
run_batch_book_v3_7.py
Batch runner for BOOK subscribers using the v3.7 unified engine.
"""
...
SUB_DIR  = os.path.join(BASE_DIR, "data", "subscribers", "BOOK")
...
def run_batch(start_date, end_date):
    print(f"\n[BOOK BATCH v3.7] RUNNING BOOK FORECASTS {start_date} → {end_date}\n")
    ...
    for sub_file in subscribers:
        ...
        print(f"[BOOK BATCH v3.7] Processing: {sub_file}")
        print(f"[CMD] {' '.join(cmd)}")

        subprocess.run(cmd, cwd=BASE_DIR)

    print("\n[BOOK BATCH v3.7] COMPLETED.\n")
