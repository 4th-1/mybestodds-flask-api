...
Usage:
    python run_kit_v3_7.py KITNAME START_DATE END_DATE SUBSCRIBER_JSON

Example:
    python run_kit_v3_7.py BOOK3 2025-12-05 2025-12-07 data/subscribers/BOOK3/JDS_BOOK3.json
"""
...
PROJECT_ROOT = SCRIPT_DIR   # THIS FILE LIVES DIRECTLY IN PROJECT ROOT

if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# --- FIXED IMPORT (old version incorrectly referenced engine_core_v3_7) ----
...
def write_csv(path: str, rows: List[Dict[str, Any]]) -> None:
    """
    Dynamic CSV writer discovering all keys automatically.
    Prevents failures when new v3.7 fields appear.
    """
    ensure_folder(os.path.dirname(path))
...
def main():
    ...
    # --- FIX: CONFIG PATH BASED ON SHOWN DIRECTORY STRUCTURE ---
    config_path = os.path.join(PROJECT_ROOT, "config", "config_v3_7.json")

    # Resolve subscriber path
    subscriber_path = SUB_PATH if os.path.isabs(SUB_PATH) else os.path.join(PROJECT_ROOT, SUB_PATH)
    ...
    # Load config & subscriber
    config = load_json(config_path)
    subscriber = load_json(subscriber_path)

    # --- FIX: USE REAL ENGINE YOU HAVE INSTALLED ---
    ...
    print(f"[INFO] Rows passed v3.7 pipeline (Rubix + BOB + Option C + Legend).\n")
    ...
    # Write full forecast
    write_json(os.path.join(out_dir, "forecast.json"), rows)
    write_csv(os.path.join(out_dir, "forecast.csv"), rows)
    ...
    write_json(os.path.join(out_dir, "audit_preview.json"), audit_preview)

    print("[SUCCESS] Output saved at:")
    print(f"         {out_dir}")
    print("[READY] Run SENTINEL on forecast.json or audit_preview.json.\n")
